...
#define DOCTEST_STATIC_ASSERT(expression, message)                                               \
    struct DOCTEST_CAT(__static_assertion_at_line_, __LINE__) {                                  \
        doctest::detail::static_assert_impl::StaticAssertion<static_cast<bool>((expression))>    \
            DOCTEST_CAT(DOCTEST_CAT(DOCTEST_CAT(STATIC_ASSERTION_FAILED_AT_LINE_, __LINE__), _), \
                        message);                                                                \
    };                                                                                           \
...
inline bool operator>=(const String& lhs, const String& rhs) { return (lhs != rhs) ? lhs.compare(rhs) > 0 : true; }
    ...
    template<class T> struct enable_if<true, T> { typedef T type; };
        ...
        struct TestData {
            ...
            TestData(const char* suite, const char* name, funcType f, const char* file,
                     unsigned line)
...
#pragma warning(pop)
    ...
    template <class L, class R> struct RelationalComparator<5, L, R> { bool operator()(const DOCTEST_REF_WRAP(L) lhs, const DOCTEST_REF_WRAP(R) rhs) const { return le(lhs, rhs); } };
        ...
        struct ResultBuilder {
            ...
            ResultBuilder(assertType::Enum assert_type, const char* file, int line,
...
// to fix gcc 4.7 "-Winline" warnings
        ...
        int fast_binary_assert(assertType::Enum assert_type, const char* file, int line,
            ...
            // #########################################################################################
            // IF THE DEBUGGER BREAKS HERE - GO 1 LEVEL UP IN THE CALLSTACK TO SEE THE FAILING
            // ASSERTION
...
namespace doctest {
    ...
    String::String(const String& other) : m_str(0) { copy(other); }
    ...
    String& String::operator+=(const String& other) {
        ...
        if (other.m_str != 0) {
            char* newStr =
                static_cast<char*>(malloc(my_strlen(m_str) + my_strlen(other.m_str) + 1));
            strcpy(newStr, m_str);
    ...
    int String::compare(const char* other, bool no_case) const {
        if (no_case) return detail::stricmp(m_str, other);
        return strcmp(m_str, other);
    }

    int String::compare(const String& other, bool no_case) const {
        if (no_case) return detail::stricmp(m_str, other.m_str);
...
namespace doctest {
    namespace detail {
        ...
        bool checkIfShouldThrow(assertType::Enum assert_type) {
            if (assert_type & assertType::is_require) return true;
        ...
        void fastAssertThrowIfFlagSet(int flags) {
            if (flags & assertAction::shouldthrow) throwException();
        }
        void throwException() {
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
        ...
        int matchesAny(const char* name, std::vector<String> filters, int matchEmpty,
                       bool caseSensitive) {
            if (filters.size() == 0 && matchEmpty) return 1;
            for (unsigned i = 0; i < filters.size(); ++i)
                if (wildcmp(name, filters[i].c_str(), caseSensitive)) return 1;
            return 0;
        }
        ...
        bool SubcaseSignature::operator<(const SubcaseSignature& other) const {
            if (m_line != other.m_line) return m_line < other.m_line;
            if (strcmp(m_file, other.m_file) != 0) return strcmp(m_file, other.m_file) < 0;
        ...
        Subcase::~Subcase() {
            if (m_entered) {
                ...
                if (s->subcasesStack.size() > 0) s->subcasesStack.pop_back();
                if (s->hasLoggedCurrentTestStart) logTestEnd();
                s->hasLoggedCurrentTestStart = false;
            }
        }
...
#undef DOCTEST_SET_ATTR
        ...
        // this is needed because MSVC does not permit mixing 2 exception handling schemes in a
        // function
...
#endif  // DOCTEST_CONFIG_NO_EXCEPTIONS
                f();
                if (getContextState()->numFailedAssertionsForCurrentTestcase) res = EXIT_FAILURE;
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
...
#include <sys/sysctl.h>
        ...
        void logTestException(const char* what) {
            ...
            DOCTEST_SNPRINTF(msg, DOCTEST_COUNTOF(msg),
                             "TEST CASE FAILED! (threw exception: %s)\n\n", what);
        ...
        bool parseCommaSepArgs(int argc, const char* const* argv, const char* pattern,
            ...
            if (parseOption(argc, argv, pattern, filtersString)) {
                ...
                while (pch != 0) {
                    ...
                    pch =
                        strtok(0, ",");  // uses the strtok() internal state to go to the next token
                }
                return true;
            }
            return false;
        ...
        void printHelp() {
            ...
            printf(
                " -nv,  --no-version=<bool>             omit the framework version in the "
                "output\n");
...
#undef DOCTEST_PARSE_AS_BOOL_OR_FLAG
    ...
    int Context::run() {
        ...
        for (i = 0; i < testArray.size(); i++) {
            ...
            if (!matchesAny(data.m_suite, p->filters[2], 1, p->case_sensitive)) continue;
            if (matchesAny(data.m_suite, p->filters[3], 0, p->case_sensitive)) continue;
...
//__try {
                ...
                do {
                    ...
                    // reset some of the fields for subcases (except for the set of fully passed
                    // ones)
...
//}
        ...
        if (numFailed && !p->no_exitcode) return EXIT_FAILURE;
        return EXIT_SUCCESS;
    }
}  // namespace doctest
